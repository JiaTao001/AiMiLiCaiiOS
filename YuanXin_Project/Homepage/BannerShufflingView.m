...
- (void)updateConstraints {
    ...
    if (self.contentView.translatesAutoresizingMaskIntoConstraints) {
        ...
        NSArray *pageH = [NSLayoutConstraint constraintsWithVisualFormat:@"H:[_pageControl]-20-|" options:0 metrics:nil views:NSDictionaryOfVariableBindings(_pageControl)];
...
- (void)layoutSubviews {
    ...
    if ( self.allImageViews.count && !CGSizeEqualToSize([self.allImageViews.firstObject bounds].size, self.contentView.size) ) {
        for (UIImageView *imageView in self.allImageViews) {
            
            if ( [imageView isKindOfClass:[UIImageView class]] ) {
                imageView.frame = CGRectMake(imageView.tag*self.contentView.width, 0, self.contentView.width, self.contentView.height);
            }
        }
        self.contentView.contentSize = CGSizeMake(self.allImageViews.count*self.contentView.width, self.contentView.height);
        if ( 1 != self.allImageViews.count)
            self.contentView.contentOffset = CGPointMake(self.allImageViews.count/2*self.contentView.width, 0);
    }
}
...
- (void)clickImageView:(UITapGestureRecognizer *)tapGesture {
    ...
    if ([self.allImageViews containsObject:tapGesture.view]) {
        ...
        if (self.clickBlock) {
            @weakify(self)
            self.clickBlock(self_weak_, [self convertIndexToRow:tapGesture.view.tag]);
        }
    }
}
...
- (void)setCurrentRow:(NSInteger)currentRow {
    ...
    if (MAX_VIEW_NUM == self.allImageViews.count /* && ![self.visiableImageViews containsObject:self.allImageViews[self.allImageViews.count/2]] */) {
        ...
        if (!imageView.image && [self.dataSource respondsToSelector:@selector(imageLoop:visiableImageView:imageForRow:)]) {//没有图片就调用一次代理获取图片
            
            [self.dataSource imageLoop:self visiableImageView:imageView imageForRow:currentRow];
        }
    }
    self.contentView.contentOffset = CGPointMake(self.allImageViews.count/2*self.contentView.bounds.size.width, 0);
    
    //是否需要调整坐标
    if (currentRow == _currentRow) return;
