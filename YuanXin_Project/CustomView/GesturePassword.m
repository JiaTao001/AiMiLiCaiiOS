...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        _gradientColor = NULL;
        _normalColor = cycle_Color;
        _highlightColor = Theme_Color;
    }
...
- (void)drawRect:(CGRect)rect {
    ...
    CGRect marginRect = CGRectMake(0.5, 0.5, rect.size.width - 1, rect.size.height - 1);
    ...
    if (self.selected) {
        ...
        //内弧
//        CGFloat radius1 = rect.size.width/4; //内圆半径为外圆的1/4
//        CGMutablePathRef path1 = CGPathCreateMutable();
//        CGPathAddArc(path1, NULL, rect.size.width/2, rect.size.height/2, radius1, M_PI*10/6.0, M_PI*2/6.0,0);
...
//        
        //内弧
//        CGFloat radius2 = rect.size.width/4; //内圆半径为外圆的1/4
//        CGMutablePathRef path2 = CGPathCreateMutable();
//        CGPathAddArc(path2, NULL, rect.size.width/2, rect.size.height/2, radius2, M_PI*4/6.0, M_PI*8/6.0,0);
//        //        CGContextAddArc(ctx, rect.size.width/2, rect.size.height/2, radius, 0, 2*M_PI, 0);
...
//        CGContextDrawPath(ctx, kCGPathStroke);
        ...
        //内圆
        CGFloat radius = rect.size.width/8; //内圆半径为外圆的1/4
...
//        //画出外圆
//        CGContextMoveToPoint(ctx, , 300);//设置Path的起点
...
//        CGGradientRelease(gradient),gradient=nil;
        ...
        CGContextSetStrokeColorWithColor(ctx, cycle_Color.CGColor);

        //外圆
//        CGContextAddEllipseInRect(ctx, marginRect);
//        CGContextDrawPath(ctx, kCGPathStroke);
        CGFloat radius1 = rect.size.width*3/8; //内圆半径为外圆的1/4
        CGMutablePathRef path1 = CGPathCreateMutable();
        CGContextSetFillColorWithColor(ctx, cycle_Color.CGColor);
        CGPathAddArc(path1, NULL, rect.size.width/2, rect.size.height/2, radius1, 0, 2*M_PI, 0);
        //        CGContextAddArc(ctx, rect.size.width/2, rect.size.height/2, radius, 0, 2*M_PI, 0);
        CGContextAddPath(ctx, path1);
        CGPathRelease(path1);
        CGContextDrawPath(ctx, kCGPathStroke);
        ...
        //内圆
        CGFloat radius = rect.size.width*2.5/8; //内圆半径为外圆的1/4
        CGMutablePathRef path = CGPathCreateMutable();
        CGContextSetFillColorWithColor(ctx, cycle_Color.CGColor);
        CGPathAddArc(path, NULL, rect.size.width/2, rect.size.height/2, radius, 0, 2*M_PI, 0);
//        CGContextAddArc(ctx, rect.size.width/2, rect.size.height/2, radius, 0, 2*M_PI, 0);
...
- (void)initializeGesturePassword {
    ...
    _unSelectedColor = cycle_Color;
    _selectedColor = Theme_Color;
    _lineColor = Theme_Color;
    _values = [self defaulrValues];
    self.backgroundColor = [UIColor clearColor];
    self.clipsToBounds = NO;
    [self addSubview:self.line];
    [self resetPasswordPart];
}
...
- (void)resetPasswordPart {
    
    for (GestureItem *item in self.items) {
        
        item.passwordPart = self.values[item.tag];
    }
}
